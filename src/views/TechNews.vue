<template>
  <div>
    <div class="bn">
      <h2 class="bn__bn">{{ $t('tech') }}</h2>
        <div class="bn__box" v-for="(news, index) in technologyNews" :key="index">
          ...
          <div class="bn__box--right">
            ...
            <router-link :to="'/tech/article/' + (index + knifePoint)"
            @click.native="recentlyViewed(news)" class="bn__box--cta">{{ $t('readArticle') }}</router-link>
          </div>
      </div>
      <div class="pages">
        <span v-for="n in (numOfTechnologyNews / 4)" :key="n" @click="knifePoint = ((n-1) * 4)">{{ n }}</span>
      </div>
    </div>
...
export default {
  ...
  computed: {
    numOfTechnologyNews () { return this.$store.state.technologyNews.length },
    technologyNews () { return this.$store.state.technologyNews.slice(this.knifePoint, this.knifePoint + 4) }
  },
  methods: {
    recentlyViewed (news) {
