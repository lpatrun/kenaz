<template>
  <div>
    <div class="bn">
      <h2 class="bn__bn">{{ $t('health') }}</h2>
        <div class="bn__box" v-for="(news, index) in healthNews" :key="index">
          ...
          <div class="bn__box--right">
            ...
            <router-link :to="'/health/article/' + (index + knifePoint)"
            @click.native="recentlyViewed(news)" class="bn__box--cta">{{ $t('readArticle') }}</router-link>
          </div>
      </div>
      <div class="pages">
        <span v-for="n in (numOfHealthNews / 4)" :key="n" @click="knifePoint = ((n-1) * 4)">{{ n }}</span>
      </div>
    </div>
...
export default {
  ...
  computed: {
    numOfHealthNews () { return this.$store.state.healthNews.length },
    healthNews () { return this.$store.state.healthNews.slice(this.knifePoint, this.knifePoint + 4) }
  },
  methods: {
    recentlyViewed (news) {
