<template>
  <div>
    <div class="bn">
      <h2 class="bn__bn">{{ $t('sport')}}</h2>
        <div class="bn__box" v-for="(news, index) in sportNews" :key="index">
          ...
          <div class="bn__box--right">
            ...
            <router-link :to="'/sport/article/' + (index + knifePoint)"
            @click.native="recentlyViewed(news)" class="bn__box--cta">{{ $t('readArticle') }}</router-link>
          </div>
      </div>
      <div class="pages">
        <span v-for="n in (numOfSportNews / 4)" :key="n" @click="knifePoint = ((n-1) * 4)">{{ n }}</span>
      </div>
    </div>
...
export default {
  ...
  computed: {
    numOfSportNews () { return this.$store.state.sportNews.length },
    sportNews () { return this.$store.state.sportNews.slice(this.knifePoint, this.knifePoint + 4) }
  },
  methods: {
    recentlyViewed (news) {
